...
from TCP.utils import calculate_angle_between_vectors
...
def connect_to_responder(host, port):
	sock = create_socket()
	while True:
		try:
...
print("Trying to connect to Responder...")
initiator_socket = connect_to_responder(host, port)

# def listen_for_responder(host, port):
#     sock = create_socket()
#     sock.bind((host, port))
#     sock.listen(1)
...
class LLVM_Agent(autonomous_agent.AutonomousAgent):
	def setup(self, path_to_conf_file, ckpt=None):
		...
		if SAVE_PATH is not None:
			...
			(self.save_path / 'input_image').mkdir()
	...
	def _truncate_global_route_till_local_target(self, windows_size=5):
		...
		for i in range(len(self._global_route)-1):
			...
			required_waypoints.append(self._global_route[i][0])
			print("hello waypoints", self._global_route[i][0], self._global_route[i+1][0], self._ego_vehicle.get_location(), type(self._global_route[i+1][0]) )
		...
		return required_waypoints, self._global_route[0][0]
	...
	def tick(self, input_data, timestamp):
		required_waypoints, closest_waypoint = self._truncate_global_route_till_local_target()

		waypoint_forward_vectors = []  # Dictionary to store waypoints and their forward vectors

		prev_waypoint = None  # Initialize previous waypoint

		for index, waypoint in enumerate(required_waypoints):
			
			
			if prev_waypoint is not None:
				# Calculate the forward vector
				forward_vector = carla.Vector3D(
					x=waypoint.transform.location.x - prev_waypoint.transform.location.x,
					y=waypoint.transform.location.y - prev_waypoint.transform.location.y,
					z=waypoint.transform.location.z - prev_waypoint.transform.location.z
				)
				
				waypoint_forward_vectors.append(forward_vector)

			# Update previous waypoint
			prev_waypoint = waypoint

		print("imp1", waypoint_forward_vectors)

		averaged_direction_vector_of_waypoint = turn_utils.average_vectors(waypoint_forward_vectors)

		print("imp2",averaged_direction_vector_of_waypoint)


		min_distance_from_waypoint = turn_utils.perpendicular_distance_from_line_to_point(averaged_direction_vector_of_waypoint, 
																									closest_waypoint.transform.location, 
																									self._ego_vehicle.get_transform().location)

		print("imp3",min_distance_from_waypoint)

		angle_between_vectors, direction = turn_utils.calculate_angle_between_vectors(self._ego_vehicle.get_transform().rotation.get_forward_vector(), averaged_direction_vector_of_waypoint)

		print("imp4",angle_between_vectors, direction)
  
		angle_between_vectors_send = angle_between_vectors

		angle_between_vectors = direction*angle_between_vectors


		if angle_between_vectors > 70:
			angle_between_vectors = 70
		
		elif angle_between_vectors < -70:
			angle_between_vectors = -70
			
		angle_between_vectors = angle_between_vectors/70.0

		correct_steering_angle = angle_between_vectors
		

		birdview_obs = self.birdview_obs_manager.get_observation(self._global_route)
		print("hello am here",self._ego_vehicle.get_transform().location)
		...
		result['steering_angle'] = correct_steering_angle
		result["angle_of_turn"] = angle_between_vectors_send
	...
	def run_step(self, input_data, timestamp):
		...
		gps = self._get_position(tick_data) # SEND x,y

		

		near_node, near_command = self._waypoint_planner.run_step(gps) # SEND near_node, near_command
		far_node, far_command = self._command_planner.run_step(gps)    # SEND far_node, far_command

		# "x_command_near": -63.66741171762685,
		# "y_command_near": 88.57479870471019,
		# "command_near": 4,
		# "should_brake": false,
		# "x_target": -110.155368798631,
		# "y_target": 88.38674938804763,
		# "target_command": 4

		# additional collision detection to enhance safety
		should_brake = self.collision_detect()
  
		if tick_data["angle_of_turn"]  > 20.0 and tick_data['speed']>4:
			should_brake=True
	
		SEND_DATA = {}

		Image.fromarray(tick_data['rgb']).save(self.save_path / 'input_image' / ('%04d.png' % self.step))

		SEND_DATA["rgb"]= str(self.save_path / 'input_image' / ('%04d.png' % self.step)) # SEND Image
		SEND_DATA['x']= tick_data['gps'][0]  # SEND x
		SEND_DATA['y']= tick_data['gps'][1]
		SEND_DATA['theta']= tick_data['compass']
		SEND_DATA['speed']= tick_data['speed'] # Speed
		SEND_DATA["x_command_near"] = near_node[0]
		SEND_DATA["y_command_near"] = near_node[1]
		SEND_DATA["command_near"] = near_command.value
		SEND_DATA['should_brake']= should_brake
		SEND_DATA["x_target"] = tick_data['x_target']
		SEND_DATA["y_target"] = tick_data['y_target']
		SEND_DATA["target_command"] = tick_data['next_command']
		...
		initiator_socket.sendall(message.encode())

		control_data = initiator_socket.recv(1024)

		control_data = json.loads(control_data.decode())

		llvm_control = carla.VehicleControl()
		llvm_control.steer = 0.95*tick_data['steering_angle']
		if not should_brake and tick_data['speed'] < 3 and llvm_control.throttle < 0.4 :
			llvm_control.throttle = 1.0
		else :
			llvm_control.throttle = control_data["throttle"]
		llvm_control.brake = control_data["brake"]
  
		
  
		self.last_control = llvm_control
		...
		# self.save(near_node, far_node, near_command, far_command, tick_data, render_img, should_brake)
